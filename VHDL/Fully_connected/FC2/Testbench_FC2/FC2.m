...
fileOUTPUT_in = fopen('ColumnOutput_dense_2.txt','r');

%open file to be written: Input, biases, Weights and Output (data in binaryl form)
% type ColumnBias_dense_2.txt   %to display the content of the file 
fileW_out = fopen('Bin_ColumnWeights_dense_2.txt','w');
fileB_out = fopen('Bin_ColumnBias_dense_2.txt','w');
fileINPUT_out = fopen('Bin_Input_from_prev_layer_2.txt','w');
fileOUTPUT_out = fopen('Bin_Output_dense_2.txt','w');
fileOUTPUT_matlab = fopen('Bin_Output_dense_2_matlab.txt','w');
...
%%  Generate gloden output of FC2 to be compared with those from Modelsim
...
[Out_fc2,Out_fc2_bin]=FC2_function_speed(parallelism_out,Inputs_fixed);

len=size(Out_fc2_bin);
for i=1:len(1)
    fprintf(fileOUTPUT_matlab,'%s',Out_fc2_bin(i,:));
...
file_from_simulation = fopen('Bin_output_simulation_dense2.txt','r');
...
figure(1)
error_Modelsim= abs(double(fi_simulation_res') - double(Out_fc2));
for i=1:length(Out_fc2)
    if Out_fc2(i)~=0
        error_Modelsim_perc(i)=(error_Modelsim(i)./double(Out_fc2(i)))*100;
    else 
        error_Modelsim_perc(i)=0;
    end
end
plot( error_Modelsim_perc,'o-')
title('Output of Modelsim vs output of MATLAB error [%]  - FC2');
ylim([-100 100]);
xlabel('Output Neurons');
ylabel('Error Modelsim vs MATLAB [%]');

% compare output of simulation with output from KERAS
%RELU of keras output
...
for i=1:length(Outputs)
    if Outputs(i)~=0
        error_keras_perc(i)=(error_keras(i)./Outputs(i))*100;
    else 
...
title('Output of Modelsim vs output of KERAS error [%] - FC2');
ylim([0 100]);
xlabel('Output Neurons');
...
figure(3)
plot(error_keras,'o-')
title('Output of Modelsim vs output of KERAS absolute error- FC2');
ylim([-0.5 1]);
xlabel('Output Neurons');
