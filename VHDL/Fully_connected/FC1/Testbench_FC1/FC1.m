...
% type ColumnBias_dense_1.txt   %to display the content of the file 
fileW_out = fopen('Bin_ColumnWeights_dense_1.txt','w');
fileB_out = fopen('Bin_ColumnBias_dense_1.txt','w');
fileINPUT_out = fopen('Bin_Input_from_prev_layer_1.txt','w');
fileOUTPUT_out = fopen('Bin_Output_dense_1.txt','w');
fileOUTPUT_matlab = fopen('Bin_Output_dense_1_matlab.txt','w');
...
[Out_fc1,Out_fc1_bin]=FC1_function_speed(parallelism_out,Inputs_fixed);

len=size(Out_fc1_bin);
for i=1:len(1)
    fprintf(fileOUTPUT_matlab,'%s',Out_fc1_bin(i,:));
...
file_from_simulation = fopen('Bin_output_simulation_dense1.txt','r');
...
figure(1)
error_Modelsim= abs(double(fi_simulation_res') - double(Out_fc1));
for i=1:length(Out_fc1)
    if Out_fc1(i)~=0
        error_Modelsim_perc(i)=(error_Modelsim(i)./double(Out_fc1(i)))*100;
    else 
        error_Modelsim_perc(i)=0;
    end
end
plot( error_Modelsim_perc,'o-')
title('Output of Modelsim vs output of MATLAB error [%]  - FC1');
ylim([-100 100]);
...
title('Output of Modelsim vs output of KERAS error [%] - FC1');
...
figure(3)
plot(error_keras,'o-')
title('Output of Modelsim vs output of KERAS absolute error- FC1');
ylim([-0.5 1]);
xlabel('Output Neurons');
